...
def get(c):
    ...
    classes=[dict(r) for r in c.execute('SELECT classes.*,departments.name AS college FROM classes JOIN departments ON departments.id=classes.department_id ORDER BY departments.name,classes.name')]
    students=[dict(r) for r in c.execute('SELECT roster.*,classes.name AS class_name,departments.name AS college FROM roster JOIN classes ON classes.id=roster.class_id JOIN departments ON departments.id=classes.department_id ORDER BY roster.student_id')]
...
def save(c,body):
    ...
    if ident and not old: raise ValueError('信息不存在')
    ...
    if kind=='student':
        ...
        if not c.execute('SELECT 1 FROM classes WHERE id=? AND active=1',(parent,)).fetchone(): raise ValueError('请选择有效班级')
...
def seed(c):
    ...
    for r in items:
        ...
        cls=c.execute('SELECT id,active FROM classes WHERE department_id=? AND name=?',(dep['id'],r['class_name'])).fetchone()
        if not cls['active']:continue
...
def batch(c,body,log,actor):
    ...
    for ident in ids:
        ...
        if not row:raise ValueError('部分信息已变化，请刷新后重新选择')
